...
from src.data.core import TextCluster
...
def batch_apply_text_mask(inpainter: str, mask_canvas: np.ndarray, text_clusters_data: list[TextCluster]) -> Image.Image:
    if mask_canvas is None:
        raise ValueError("mask_canvas cannot be None")
    
    for text_cluster_data in text_clusters_data:
        if text_cluster_data.bounding_image is None:
            continue
        mask_canvas = apply_text_mask(inpainter, mask_canvas, text_cluster_data)
    
    image_mask_pil = Image.fromarray(mask_canvas)
